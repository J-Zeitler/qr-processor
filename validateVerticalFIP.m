  ...
  for dir = directions
    if valid
      ...
      if (dir == directions(1))
        ...
        patternSize = round(((dy - 1) * 2 + 1) / 3);
      else
        blackStart = y;
        for y = blackStart:dir:round(blackStart + dir*patternSize*1.5*ratioLimit(2))
          if (y < 1 || y > height)
            valid = false;
            break;
          end
          pxl = imageBW(y,x);
          if (pxl == 1)
            ratio = dir*(y-blackStart)/(patternSize*1.5);
      ...
      if valid
        whiteStart = y;
        for y = whiteStart:dir:round(whiteStart + dir*patternSize*ratioLimit(2))
          if (y < 1 || y > height)
            valid = false;
            break;
          end
          ...
          blackStart = y;
          if (pxl == 0)
            ratio = dir*(y-whiteStart)/patternSize;
            if (ratio < ratioLimit(1))
              valid = false;
            end
            break;
